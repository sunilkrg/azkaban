...
class JdbcProjectHandlerSet {
  ...
  public static class ProjectResultHandler implements ResultSetHandler<List<Project>> {
    ...
    public static final String SELECT_PROJECT_BY_ID = BASE_QUERY + "LEFT JOIN project_permissions prm ON prj.id = CAST (prm.project_id AS UNSIGNED) WHERE prj.id=?";

    // Still return the project if it has no associated permissions
    public static final String SELECT_ACTIVE_PROJECT_BY_NAME = BASE_QUERY + "LEFT JOIN project_permissions prm ON prj.id = CAST (prm.project_id AS UNSIGNED) WHERE prj.name=? AND prj.active=true";
    ...
    public static final String SELECT_ALL_ACTIVE_PROJECTS = BASE_QUERY + "JOIN project_permissions prm ON prj.id = CAST (prm.project_id AS UNSIGNED) WHERE prj.active=true";
